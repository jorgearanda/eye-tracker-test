...
def parseCMD(id, timeBlocks):
    ...
    for i in range(20, len(cmd)):  # no valid timestamps before line 20
        ...
        if v['timestamp'] < timeBlocks[currentTimeBlock][0]:
            ...
            if not isGoodTimestamp(v):
                pairHealth[currentTimeBlock] += 1
...
if __name__ == "__main__":
    ...
    sequencesSource = ""
    if len(sys.argv) != 3:
        print "Usage: python calibration.py name_of_sequences_source_file name_of_results_file"
    else:
        ...
        for seq in seqs:
            ...
            pairHealth = parseCMD(pid, defineBeginEndTimes(ts))
            outputResults(res, pid, pairHealth)
